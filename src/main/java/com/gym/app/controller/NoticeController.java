...
public class NoticeController {
	...
	public String insertUser(NoticeDto dto){
		try{
			service.insert(dto);
	...
	public String plist(@RequestParam(value="pageNum",defaultValue="1")int pageNum,String sch_type,String sch_value,Model model){
		...
		List<NoticeDto> list=service.nlist(map);
		model.addAttribute("list", list);
		model.addAttribute("pu", pu);
		model.addAttribute("sch_type", sch_type);
		model.addAttribute("sch_value", sch_value);
		return ".notice.showPageList";	
	}
	@RequestMapping("/notice/detail")
	public String detail(int ntnum,Model model) {
		NoticeDto dto=service.getinfo(ntnum);
		NoticeDto prev=service.prev(ntnum);
		NoticeDto next=service.next(ntnum);
		String savefile=service.getPic(dto.getId());
		service.addHit(ntnum);
	...
	public String updateForm(int ntnum,Model model){
		NoticeDto dto=service.getinfo(ntnum);
		model.addAttribute("dto", dto);
		return ".notice.update";
	}
	@RequestMapping(value="/notice/update",method=RequestMethod.POST)
	public String update(NoticeDto dto,int ntnum) {
		try{
			int n=service.update(dto);
			return "redirect:/notice/detail?ntnum="+ntnum;
		}catch(Exception e){
			System.out.println(e.getMessage());
			return ".notice.error";
		}
	...
	public String delete(int ntnum) {
		try{
			int n=service.delete(ntnum);
			return ".notice.success";
		}catch(Exception e){
			System.out.println(e.getMessage());
			return ".notice.error";
	...
	@RequestMapping(value="/notice/photoUpload",method=RequestMethod.POST)
	...
	//다중파일업로드
	@RequestMapping(value="/notice/multiplePhotoUpload",method=RequestMethod.POST)
