...
$(function(){
	...
	$('#signupnext').click(function(){
		if($('#join_user_agree').prop("checked") == true && $('#join_priv_agree').prop("checked") == true){
			$('#signupconfirmform').modal('hide');
			$("#mySignUpForm").modal();
		}else{
			console.log("동의하기 체크 실패");
		}
	});
...
<div id="loginSuccess" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content alert alert-success">
		  <strong>성공!</strong> 로그인에 성공했습니다.
    </div>
  </div>
</div>
<div id="loginSuccessFail" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content alert alert-danger">
		  <strong>실패!!</strong> 아이디, 이름, 이메일이 틀렸습니다.
    </div>
...
<a href="/">
  <img src="/resources/img/brokenpipe.png" width="100%">
</a>
<nav class="navbar navbar-default">
  <div class="container-fluid">
      <ul class="menu nav navbar-nav">
        ...
        <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">Broken Pipe<b class="caret"></b></a>
          <ul class="dropdown-menu">
            <li><a href="/info/welcome">소개글</a></li>
            <!-- <li><a href="/instructor">강사소개</a></li> -->
            <li><a href="/info/location">오시는 길</a></li>
          </ul>
        </li>
        <li><a href="/notice/nlist">공지사항</a></li>
        <li><a href="/info/info">이용안내</a></li>
        <li><a href="/exer/allexerlist">헬스정보</a></li>
        <li class="dropdown">
          <a href="/qna/plist" class="dropdown-toggle" data-toggle="dropdown">게시판<b class="caret"></b></a>
          <ul class="dropdown-menu">
            <li><a href="/fboard/plist">자유 게시판</a></li>
            <li><a href="/qna/plist">문의 게시판</a></li>
            <li><a href="/infoboard/plist">건강 정보</a></li>
            <li><a href="/epboard/plist">후기 게시판</a></li>
          </ul>
        </li>
        <c:choose>
        <c:when test="${grade <= 3 }">
        <li><a href="/locker/situation">라커 관리</a></li>
        <li class="dropdown">
          <a class="dropdown-toggle" data-toggle="dropdown" href="#">직원관리 <b class="caret"></b></a>
          <ul class="dropdown-menu">
            <li><a href="#"></a></li>
        ...
        <li><a href="/analysis"><span class="glyphicon glyphicon-dashboard"></span>통계</a></li>
        </c:when>
        </c:choose>
      </ul>
...
<!-- 회원가입 약관 -->
<div class="modal fade" id="signupconfirmform" role="dialog">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="form-group pull-left">
							...
							<div class="col-xs-12">
								<textarea class="form-control" readonly="" rows="10" cols="100" style="resize: none;">제1조(목적)
...
② "홈페이지"와 이용자간에 제기된 전자거래 소송에는 한국법을 적용합니다.
							...
							<div class="col-xs-12">
								<div class="checkbox pull-right">
									<label for="join_user_agree">
										<input type="checkbox" id="join_user_agree">
						...
						<div class="form-group pull-left">
							...
							<div class="col-xs-12">
								<textarea class="form-control" readonly="" rows="10" cols="100" style="resize: none;">&lt;회사&gt;('사이트 URL'이하 '사이트명')은(는) 개인정보보호법에 따라 이용자의 개인정보 보호 및 권익을 보호하고 개인정보와 관련한 이용자의 고충을 원활하게 처리할 수 있도록 다음과 같은 처리방침을 두고 있습니다.
							...
							<div class="col-xs-12">
								<div class="checkbox pull-right">
									<label for="join_priv_agree">
										<input type="checkbox" id="join_priv_agree">
					...
					<div class="panel-footer">
						<div style="overflow: hidden">
							<div class="col-sm-offset-3 col-sm-6">
								<button class="btn btn-md btn-primary btn-block" id="signupnext">다음단계로</button>
							</div>
						</div>
					</div>
				</div>
        ...
        <div class="modal-footer">
          <button type="submit" class="btn btn-danger btn-default pull-left" data-dismiss="modal"><span class="glyphicon glyphicon-remove"></span> 취소</button>
      	  ...
      	  <c:otherwise>
      	  	<c:choose>
      	  		<c:when test="${grade==1 }">
      	   			<%-- <li><span>${id } 관리자님 환영합니다</span></li> --%>
      	   			<li><!-- 전체관리페이지 -->
			      	  	<a href="#">
			         		<span class="glyphicon glyphicon-king"> </span>${id } 관리자님
			        	</a>
			      	</li>
      	   	  	</c:when>
      	  		<c:when test="${grade==2 }">
      	  	   	  	...
      	  	   	  	<li><!-- 회원관리페이지 -->
			      	  	<a href="#">
			         		<span class="glyphicon glyphicon-bishop"> </span>${id } 사원님
			        	</a>
			      	</li>
      	  	  	</c:when>
      	  		<c:otherwise>
      	  			...
      	  			<li>
      	  			  <a href="#"><span class="glyphicon glyphicon-pawn"> </span>${id } 회원님</a>
      	  			</li>
      	  		</c:otherwise>
      	  	</c:choose>
      	    ...
      	    %>
      	  	<script>
      	  	  $(function(){
      	      	...
      	      	window.open('/msg/userlist/'+'<%=id%>',"msgshow","directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,height=600,width=600,left=550,top=150");
      	      	///msg/userlist/${id }
      	      });
      	  	...
      	  	<!-- 마이페이지 -->
      	  	<li class="dropdown">
      	  	    <a class="dropdown-toggle" id="headermenu" data-toggle="dropdown"><span class="glyphicon glyphicon-list"></span>메뉴 <b class="caret"></b></a>
