...
<script>
$(function(){
	$("#btnWriteFboard").click(function(){
		location.href="/fboard/insert";
	});
...
<table class="table table-hover">
	...
	<c:forEach var="dto" items="${list}">
		<tr>
			<td>${dto.fbnum }</td>
			<td><a href="/fboard/detail?fbnum=${dto.fbnum}&sid=${id}">${dto.title }</a>
      ...
      <c:otherwise>
	    ...
	    <span class="dropdown-menu">
	      <li><a type="button" class="btnSend" href="#">쪽지 보내기</a>
			...
			<c:choose>
 			<c:when test="${grade==1 }">
				<td><a href="/fboard/delete?fbnum=${dto.fbnum}">삭제</a></td>
			</c:when>
			</c:choose>
	...
	<ul class="pagination">
		<c:choose>
			<c:when test="${pu.startPage>=11 }">
				<li><a href="/fboard/plist?pageNum=${pu.startPage-1 }&sch_type=${sch_type }&sch_value=${sch_value}">이전페이지</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${pu.startPage }" end="${pu.endPage }">
			<c:choose>
		    	<c:when test="${i == pu.pageNum }">
			    	<li class="active"><a href="/fboard/plist?pageNum=${i }&sch_type=${sch_type }&sch_value=${sch_value}">${i }</a></li>
		    	</c:when>
		    	<c:otherwise>
					<li><a href="/fboard/plist?pageNum=${i }&sch_type=${sch_type }&sch_value=${sch_value}">${i }</a></li>
		    	</c:otherwise>
		  	</c:choose>
		</c:forEach>
		<c:choose>
   	  		<c:when test="${pu.endPage<pu.pageCount }">
			  	<li><a href="/fboard/plist?pageNum=${pu.endPage+1 }&sch_type=${sch_type }&sch_value=${sch_value}">다음페이지</a></li>
			</c:when>
	  		<c:otherwise>
...
<!-- 검색하는 곳 -->
	...
	<div class="row">
		<div class="col-md-4 col-md-offset-4">
			<form id="form_search" method="get" action="/fboard/plist">
				<!-- input-group -->
...
<div class="text-center">
	<c:choose>
		...
		<c:otherwise>
			<button class="btn btn-primary btn-sm text-center" id="btnWriteFboard">글쓰기</button>
		</c:otherwise>
	</c:choose>
</div>
		</div>
...
<form id="form_search" method="get" action="/fboard/plist">
 ...
 <c:otherwise>
  <button class="btn btn-primary btn-sm" id="btnWriteFboard" style="float: left;">글쓰기</button>
 </c:otherwise>
</c:choose>
</div>
 -->
