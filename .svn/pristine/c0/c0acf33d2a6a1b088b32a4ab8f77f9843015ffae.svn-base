...
<div class="card card-inverse card-primary">
  <div class="card-block">
    <blockquote class="card-blockquote">
      <p><a href="/notice/detail?ntnum=${notice.ntnum}">${notice.title }</a></p>
      <footer>${notice.regdate } <cite title="Source Title">${notice.id }</cite></footer>
    </blockquote>
  </div>
</div>
</c:forEach>
...
<c:forEach var="dto" items="${list}">
		...
		<tr>
			<td>${dto.ntnum }</td>
			<td><a href="/notice/detail?ntnum=${dto.ntnum}">${dto.title }</a>
...
<div class="text-center">
	<c:choose>
		...
		<c:otherwise>
			<button class="btn btn-primary btn-sm text-center" id="btnWriteNot">글쓰기</button>
		</c:otherwise>
	</c:choose>
</div>
		</div>
...
<c:choose>
 <c:when test="${grade==1 }">
	<a href="/notice/insert" class="btn btn-primary btn-sm" role="button" style="float: left;">글쓰기</a>
 </c:when>
</c:choose>
</div>
 -->
