...
import com.gym.app.service.FboardService;
...
public class FboardController {
	@Autowired private FboardService service;
	@RequestMapping(value="/fboard/insert",method=RequestMethod.GET)
	public String insertForm(){
		return ".fboard.insert";
	}
	@RequestMapping(value="/fboard/insert",method=RequestMethod.POST)
	public String insertUser(FboardDto dto){
		try{
			int n=service.insert(dto);
			return ".fboard.success";
		}catch(Exception e){
			System.out.println(e.getMessage());
			return ".fboard.error";
		}
	}
	@RequestMapping("/fboard/plist")
	public String plist(@RequestParam(value="pageNum",defaultValue="1")int pageNum,String sch_type,String sch_value,Model model){	
		...
		List<FboardDto> list=service.plist(map);
		model.addAttribute("list", list);
		model.addAttribute("pu", pu);
		model.addAttribute("sch_type", sch_type);
		model.addAttribute("sch_value", sch_value);
		return ".fboard.showPageList";	
	}
	@RequestMapping("/fboard/detail")
	public String detail(int fbnum,String sid,Model model) {
		FboardDto dto=service.getinfo(fbnum);
		FboardDto prev=service.prev(fbnum);
		FboardDto next=service.next(fbnum);
		String savefile=service.getPic(dto.getId());
		if(sid!=null){
			String savefile1=service.getPic(sid);
			model.addAttribute("savefile1", savefile1);
			}
		service.addHit(fbnum);
	...
	public String updateForm(int fbnum,Model model){
		FboardDto dto=service.getinfo(fbnum);
		model.addAttribute("dto", dto);
		return ".fboard.update";
	}
	@RequestMapping(value="/fboard/update",method=RequestMethod.POST)
	public String update(FboardDto dto,int fbnum) {
		try{
			int n=service.update(dto);
			return "redirect:/fboard/detail?fbnum="+fbnum;
		}catch(Exception e){
			System.out.println(e.getMessage());
			return ".fboard.error";
		}
	}
	@RequestMapping("/fboard/delete")
	public String delete(int fbnum) {
		try{
			int n=service.delete(fbnum);
			return ".fboard.success";
		}catch(Exception e){
			System.out.println(e.getMessage());
			return ".fboard.error";
	...
	@RequestMapping(value="/fboard/photoUpload",method=RequestMethod.POST)
	...
	//다중파일업로드
	@RequestMapping(value="/fboard/multiplePhotoUpload",method=RequestMethod.POST)
