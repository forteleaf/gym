			...
			<form role="form" id="msgSendForm">
				<div class="modal-body">
					...
					<input type="hidden" value="${id }" id="id" name="id"> <input
						type="hidden" value="" id="recv_id" name="recv_id">
...
<div class="well-bg bg-info">
	...
	<table class="table table-hover">
		...
		<c:forEach var="dto" items="${list}">
			...
			<tr>
				<td></td>
				<td>${dto.qnanum }</td>
				<td><a href="/qna/detail?qnanum=${dto.qnanum}&sid=${id}">${dto.title }</a>
				<c:if test="${toDate eq dto.regdate}">
						<span class="label label-warning label-xs">New</span>
					</c:if></td>
				<td><c:choose>
						<c:when test="${grade >= 1}">
							<span class="dropdown" style=""> <a
								class="dropdown-toggle" data-toggle="dropdown" href="#"
								onclick="$('#recv_id').val('${dto.id}')">${dto.id }</a> <span
								class="dropdown-menu">
						...
						<c:otherwise>
							<span class="dropdown" style=""> ${dto.id } <span
								class="dropdown-menu">
									<li><a type="button" class="btnSend" href="#">쪽지 보내기</a>
							</span>
							</span>
				...
				<ul class="pagination">
					<c:choose>
						<c:when test="${pu.startPage>=11 }">
							<li><a
								href="/qna/plist?pageNum=${pu.startPage-1 }&sch_type=${sch_type }&sch_value=${sch_value}">이전페이지</a></li>
						</c:when>
						<c:otherwise>
					...
					<c:forEach var="i" begin="${pu.startPage }" end="${pu.endPage }">
						<c:choose>
							<c:when test="${i == pu.pageNum }">
								<li class="active"><a
									href="/qna/plist?pageNum=${i }&sch_type=${sch_type }&sch_value=${sch_value}">${i }</a></li>
							</c:when>
							<c:otherwise>
								<li><a
									href="/qna/plist?pageNum=${i }&sch_type=${sch_type }&sch_value=${sch_value}">${i }</a></li>
					...
					<c:choose>
						<c:when test="${pu.endPage<pu.pageCount }">
							<li><a
								href="/qna/plist?pageNum=${pu.endPage+1 }&sch_type=${sch_type }&sch_value=${sch_value}">다음페이지</a></li>
						</c:when>
						<c:otherwise>
	...
	<div class="row">
		<div class="col-md-4 col-md-offset-4">
			<form id="form_search" method="get" action="/qna/plist">
				<!-- input-group -->
...
<div class="text-center">
	<c:choose>
		...
		<c:otherwise>
			<button class="btn btn-primary btn-sm text-center" id="btnWriteQna">글쓰기</button>
		</c:otherwise>
	</c:choose>
</div>
		</div>
